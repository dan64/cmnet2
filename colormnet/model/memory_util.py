...
def do_softmax(similarity, top_k: Optional[int]=None, inplace=False, return_usage=False,
    ...
    # group_mask/group_distance/alpha_norm (replaces the earlier
    # group_bias - same group_mask contract: (B, N, HW) boolean, or an
    # .expand()'ed view, gathered at the selected top-k indices below,
    # verified to work with torch.gather; group_distance is the RAW
    # temporal distance for the group's elements (same shape contract as
    # group_mask used to carry for group_bias), zero/irrelevant elsewhere -
    # unlike the old group_bias, the penalty itself is no longer
    # precomputed by the caller: it can only be computed here, after
    # top-k selection, because it is now SCALED by the spread
    # (max-min) of the survivors' own raw scores - a
    # quantity that does not exist before this point. alpha_norm is a plain
    # scalar (not a tensor): the normalized penalty strength.
    # Preserves the same guarantee as the old group_bias: this does NOT change how
    # much the group weighs in the final distribution overall - it only
    # redistributes *within* that fixed aggregate weight, favoring
    # less-penalized members. Default None -> identical behavior to before
    # this mechanism existed, for every other caller (training included,
    ...
    if top_k is not None:
        ...
        if group_mask is not None:
            ...
            # verified empirically that it does NOT
            # produce a clean, safe value on its own for an empty group).
            ...
            # split of that fixed W_group budget. Edge case (verified below,
            # not just in theory, re-verified here
            # not to regress): if is_group is all-False for a given HW
        ...
        # group rebalancing not supported here - dead branch in production,
        # left untouched.
